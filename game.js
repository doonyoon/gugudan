...
const LOGIN_BGM_ID='HdTkXL6BTSM',LOBBY_BGM_ID='xM911Syufvg',STAGE_ONE_BGM_ID='bnTiOKWAyQM',FINAL_STAGE_BGM_ID='KSDa6iyrmkM';
const MAX_DEPLOYED_CATS=30;
...
async function apiRequest(path,options={}){const headers={'Content-Type':'application/json',...(options.headers||{})};if(authToken&&!authToken.startsWith('local:'))headers.Authorization=`Bearer ${authToken}`;const response=await fetch(`${API_BASE}${path}`,{...options,headers});const data=await response.json().catch(()=>({}));if(!response.ok){const error=new Error(data.error||'서버에 연결할 수 없습니다.');error.status=response.status;throw error;}return data;}
...
async function signupAccount(){const {id,password,message}=authValues();if(!validAccount(id,password,message))return;message.textContent='서버에 계정을 만드는 중...';try{const data=await apiRequest('/api/signup',{method:'POST',body:JSON.stringify({id,password,progress:localProgressFor(id)})});message.textContent='회원가입 완료! 진행 상황이 서버에 저장됩니다.';message.className='success';activateAccount(data.id,data.token,data.progress);}catch(error){if(error.status){message.textContent=error.message;return;}const accounts=loadAccounts();if(accounts[id]){message.textContent='이미 사용 중인 아이디입니다.';return;}accounts[id]={password:await hashPassword(password),createdAt:Date.now()};localStorage.setItem(AUTH_ACCOUNTS_KEY,JSON.stringify(accounts));message.textContent='회원가입 완료! 현재 브라우저에 안전하게 저장했습니다.';message.className='success';activateAccount(id,`local:${id}`,localProgressFor(id));}}
async function loginAccount(event){event.preventDefault();const {id,password,message}=authValues();if(!validAccount(id,password,message))return;message.textContent='로그인 중...';try{const data=await apiRequest('/api/login',{method:'POST',body:JSON.stringify({id,password})});activateAccount(data.id,data.token,data.progress);}catch(error){if(id===DEVELOPER_ID&&password===DEVELOPER_PASSWORD){activateAccount(id,`local:${id}`,developerProgress());return;}const legacy=loadAccounts()[id],legacyHash=legacy?await hashPassword(password):'';if(legacy&&legacy.password===legacyHash){activateAccount(id,`local:${id}`,localProgressFor(id));return;}message.textContent=error.message;}}
...
let saveTimer;
function saveProgress(){if(!isDeveloperAccount()){localStorage.setItem(progressKey(),JSON.stringify(progress));if(!authToken.startsWith('local:')){clearTimeout(saveTimer);saveTimer=setTimeout(()=>apiRequest('/api/progress',{method:'PUT',body:JSON.stringify({progress})}).catch(error=>console.error('Server save failed:',error.message)),150);}}renderMeta();}
function renderMeta(){
